...
1. [Overview](#overview)
2. [Complete Calculation Flow](#complete-calculation-flow)
3. [Layer 1: Core Parameters](#layer-1-core-parameters)
4. [Layer 2: OPEX, Revenue & Carbon Metrics](#layer-2-opex-revenue--carbon-metrics)
5. [Layer 3: Direct OPEX & Weighted Carbon Intensity](#layer-3-direct-opex--weighted-carbon-intensity)
6. [Layer 4: Total OPEX, Emissions & LCOP](#layer-4-total-opex-emissions--lcop)
7. [Financial Analysis](#financial-analysis)
8. [Traceable Output Format](#traceable-output-format)
...
**Formula:**
```
LCOP = (TCI_annualized + OPEX_total - Revenue_byproducts) / SAF_production
```
...
```json
{
  ...
  "inputs": {
    ...
    "hydrogen_cost": { "value": 113400000.0, "unit": "USD/year" },
    "electricity_cost": { "value": 3300000.0, "unit": "USD/year" },
    "indirect_opex": { "value": 30800000.0, "unit": "USD/year" },
    "total_revenue": { "value": 1177500000.0, "unit": "USD/year" },
  ...
  "calculation_steps": [
    ...
    },
    {
      ...
      "formula": "numerator = tci_annual + feedstock + h2 + elec + indirect - revenue",
      "calculation": "37757170 + 562650000 + 113400000 + 3300000 + 30800000 - 1177500000",
      "result": { "value": -429592829.7026977, "unit": "USD/year" },
      "intermediate_sum": "747907170.30 (costs) - 1177500000 (revenue) = -429592829.70"
    },
    {
      "step": 5,
      "description": "Calculate LCOP (noting byproduct credit creates negative cost for JET)",
      "formula": "lcop = numerator / production",
      "calculation": "-429592829.70 / 500000 = -859.19",
      "result": { "value": -859.1856594053954, "unit": "USD/t" },
      "note": "Negative value indicates byproduct revenue exceeds total costs for main product (JET)"
    }
  ],
  "metadata": {
    "interpretation": "In this calculation, LCOP represents the cost allocation to the primary SAF product (JET fuel) after accounting for byproduct revenue. The negative value indicates that byproduct sales (DIESEL + Naphtha) generate more revenue than the total costs, making the JET fuel production profitable before considering its own revenue.",
    "alternative_lcop_without_byproduct_credit": {
      "value": 1495.8143405946046,
      "formula": "LCOP_alt = (TCI_annual + Total_OPEX) / Production",
      "calculation": "(37757170.30 + 710150000) / 500000 = 1495.81"
    }
  }
}
```

**Note:** The actual API returns LCOP = 1495.81 USD/t, which uses the alternative calculation **without byproduct revenue credit**:

```json
{
  "name": "LCOP (Alternative - Without Byproduct Credit)",
  "unit": "USD/t",
  "value": 1495.8143405946046,
  "formula": "LCOP = (TCI_annual + Total_OPEX) / Production",
  "inputs": {
    "tci_annual": { "value": 37757170.29730227, "unit": "USD/year" },
  ...
  "calculation_steps": [
    {
      "step": 1,
      "description": "Sum annualized costs",
      "formula": "total_annual_cost = tci_annual + total_opex",
      "calculation": "37757170.30 + 710150000 = 747,907,170.30",
      "result": { "value": 747907170.29730227, "unit": "USD/year" }
    },
    {
      "step": 2,
      "description": "Calculate LCOP",
      "formula": "lcop = total_annual_cost / production",
      "calculation": "747907170.30 / 500000 = 1495.81",
      "result": { "value": 1495.8143405946046, "unit": "USD/t" }
    }
  ]
}
```
